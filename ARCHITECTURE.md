...
- `materials`: каталог материалов и связанные admin API сценарии. `ARCH-CLEAN-2A` перенес read-сценарии в `src/supply_bot/materials/application/`; `ARCH-CLEAN-2B` перенес create family/create variant. SKU/alias create сценарии пока остаются в `admin_api/use_cases/materials.py`.
...
- `src/supply_bot/admin_api/use_cases/materials.py`: переходный HTTP-зависимый use-case слой. Read-сценарии и create family/create variant уже делегируют в `src/supply_bot/materials/application/`; create SKU/create alias нужно вынести следующими фазами.
...
- `ARCH-CLEAN-2`: materials application layer. `ARCH-CLEAN-2A` перенес read use-cases; `ARCH-CLEAN-2B` перенес create family/create variant; следующие шаги должны вынести create SKU/create alias из `admin_api/use_cases/materials.py`.
