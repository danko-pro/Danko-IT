...
- `materials`: каталог материалов и связанные admin API сценарии. `ARCH-CLEAN-2A` начал перенос read-сценариев в `src/supply_bot/materials/application/`; create/update сценарии пока остаются в `admin_api/use_cases/materials.py`.
...
- `src/supply_bot/admin_api/use_cases/materials.py`: переходный HTTP-зависимый use-case слой. Read-сценарии уже делегируют в `src/supply_bot/materials/application/`; create/update сценарии нужно вынести следующими фазами.
...
- `ARCH-CLEAN-2`: materials application layer. `ARCH-CLEAN-2A` переносит read use-cases; следующие шаги должны вынести create/update сценарии из `admin_api/use_cases/materials.py`.
