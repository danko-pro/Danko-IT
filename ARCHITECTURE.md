...
- `dashboard`: агрегированные summary/read models для админки. Должен остаться read/application сценариями поверх storage, без бизнес-логики в route. Dashboard finance engine для объектов должен быть вынесен в `src/supply_bot/projects/domain/finance.py`; UI не должен быть источником финансовых расчетов. План закреплен в `docs/DASHBOARD_FINANCE_ENGINE_PLAN.md`, следующий этап - `DASH-FINANCE-1`.
...
- `DASH-FINANCE-1`: создать чистый dashboard finance engine в `src/supply_bot/projects/domain/finance.py` и покрыть unit-тестами. UI не должен быть источником финансовых расчетов; `storage_projects/summary.py` должен стать adapter-слоем, который читает данные, вызывает domain engine и сохраняет summary.
