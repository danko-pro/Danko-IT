import { formatArea, formatMeters, formatMoney, trimFloat } from "./";
import type { CalculatorFlooringRoom, CalculatorFlooringSpecItem, FlooringStageReadyProps } from "./";

type FlooringEstimatePanelProps = Pick<FlooringStageReadyProps, "flooringPreview">;

export function FlooringEstimatePanel(props: FlooringEstimatePanelProps) {
  const { flooringPreview } = props;
  const estimateTotal = flooringPreview.specification.reduce((sum, item) => sum + item.amount, 0);
  const workItems = flooringPreview.specification.filter((item) => item.kind === "work");
  const materialItems = flooringPreview.specification.filter((item) => item.kind === "material");
  const selectedRooms = flooringPreview.rooms.filter((room) => room.selected);
      ...
      <div className="subpanel calculator-stage-section warmfloor-estimate-panel p-3">
        <div className="calculator-stage-section-head">
          <div>
            ...
            <div className="calculator-stage-section-title">Работы и материалы по покрытиям</div>
          </div>
          <div className="calculator-stage-section-note">
            {flooringPreview.specification.length
              ? `${flooringPreview.specification.length} позиций · ${formatMoney(estimateTotal)}`
        ...
        {flooringPreview.specification.length ? (
          ...
          <div className="warmfloor-estimate-empty">
            Выберите помещения и покрытия, чтобы собрать ведомость.
          </div>
        )}
      </div>
      ...
      {selectedRooms.length ? (
        <div className="subpanel calculator-stage-section warmfloor-estimate-panel p-3">
          <div className="calculator-stage-section-head">
            ...
            <div className="calculator-stage-section-note">
              Площади, закупка, покрытие, подготовка, укладка и состав стоимости по каждой комнате.
            </div>
          </div>
...
function EstimateGroup(props: { title: string; items: CalculatorFlooringSpecItem[] }) {
...
function EstimateRow(props: { item: CalculatorFlooringSpecItem }) {
  const { item } = props;
  return (
...
function RoomEstimateRow(props: { room: CalculatorFlooringRoom }) {
  const { room } = props;
  return (
    <div className="warmfloor-estimate-row-shell">
      ...
      <div className="warmfloor-estimate-children">
        <CostChild title="Покрытие" amount={room.material_cost} detail={`${trimFloat(room.purchase_area_m2)} м²`} />
        <CostChild title="Укладка" amount={room.installation_cost} detail={`${trimFloat(room.effective_area_m2)} м²`} />
        <CostChild title="Подготовка" amount={room.preparation_total_cost} />
        <CostChild title="Подложка" amount={room.underlay_cost} detail={`${trimFloat(room.underlay_qty)} м²`} />
        ...
        <CostChild title="СВП" amount={room.svp_cost} detail={`${trimFloat(room.svp_qty)} ${room.svp_unit}`} />
        <CostChild title="Затирка" amount={room.grout_cost} detail={`${trimFloat(room.grout_qty)} ${room.grout_unit}`} />
        <CostChild title="Доп. расходники" amount={room.custom_consumables_cost} />
        <CostChild title="Плинтус" amount={room.plinth_material_cost + room.plinth_install_cost} detail={formatMeters(room.plinth_m)} />
