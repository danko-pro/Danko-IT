...
Отдельный обязательный блок работ:

1. навести порядок в структуре проекта;
2. порезать раздутые файлы;
3. ввести новую модульную структуру без монолитных экранов и перегруженных state-файлов.

Что именно входит:

- проверить самые тяжёлые файлы frontend и backend;
- выделить зоны `model / state / api / ui / overlay / builder` там, где они ещё смешаны;
- вынести общую интерактивную механику из feature-кода в shared-слой;
- сократить файлы, которые уже выросли выше нормального рабочего объёма;
- убрать временные связки, которые остались после быстрого прототипирования;
- зафиксировать новую целевую структуру папок до следующего этапа логики.

Приоритет на завтра:

1. `dashboard` и связанный `projects`-слой;
2. `admin_api/projects`;
3. общие `shared/interactions` и `shared/ui`;
4. затем оставшиеся перегруженные точки.
