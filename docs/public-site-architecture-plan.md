...
##### F.package-10. Flooring package model — 10/10 контракт внедрения

> **Статус:** актуальный инженерный план после локальной реализации FS1–FS3d и UX1–UX6c (2026-06-03). Цель — довести раздел полов до production-состояния, где данные не теряются, public snapshot детерминирован, публичный калькулятор выбирает понятные пакеты, а спецификация раскрывает реальный состав материалов/расходников/работ.

**Терминология**

- **Кубик** — атом библиотеки: материал, работа, расходник или инструмент с формулой расхода (`estimate_flooring_assembly_items`).
- **Пакет** — строка публичного каталога (`covering`, `preparation`, `layout`) плюс сохранённая сборка (`estimate_flooring_catalog_assemblies` + rows). Пример: `Керамогранит 120×60`.
- **Flat projection** — агрегированные поля пакета для быстрого расчёта: материал/работа/расходники/запас/коэффициенты.
- **Spec lines** — публично безопасные строки спецификации, полученные из состава пакета: клей, грунт, СВП, затирка, покрытие, работа и т.д.

**Целевой поток данных**

```mermaid
flowchart TB
  Cubes["Library cubes\nmaterials / works / consumables / tools"]
  Package["Catalog package\ncovering / preparation / layout"]
  Flat["Flat projection\nrates for calculation"]
  Spec["Public spec lines\nsafe estimate rows"]
  Snapshot["flooring snapshot\nbuild-time JSON"]
  Public["/estimate\nselector + total + downloadable spec"]
...
1. `covering` хранит материалы/расходники/инструмент, но **не** хранит работу укладки.
2. `layout` хранит работу укладки, `laborFactor` и дополнительный запас.
3. `preparation` в текущем публичном контуре хранит только работы/ставки подготовки; материалы подготовки не расширяются без отдельного решения.
4. Public UI остаётся простым: помещение выбирает `покрытие → подготовка → укладка`; пользователь не видит кубики.
5. Public snapshot не отдаёт internal-поля: DB id, owner, note, source, raw internal consumption, timestamps.
6. Public calculator не ходит в backend runtime; `/estimate` использует только build-time generated snapshot.
7. Snapshot-коды должны быть стабильными. Матчинг по `title` допустим только как временный fallback, не как долгосрочный идентификатор.
8. Удаление каталожного пакета не должно оставлять orphan assembly.
9. Любая миграция/backfill должна быть идемпотентной и не удалять старые flat-данные до зелёного parity.
10. Суммы public calculator не меняются без явного golden/parity обновления.

**As-built на 2026-06-03**

| Слой | Что уже есть | Ограничение |
|---|---|---|
| DB schema | `estimate_flooring_assembly_items`, `estimate_flooring_catalog_assemblies`, `estimate_flooring_catalog_assembly_rows` | `catalog_assemblies.target_kind + target_id` polymorphic, FK на covering/preparation/layout нет |
| Backend API | CRUD покрытий/подготовок/укладок, CRUD кубиков, GET/PUT/DELETE assembly | Delete target не гарантирует cleanup assembly |
| Admin UI | Сборка строки из кубиков, сохранение состава на create/edit, загрузка состава на edit, promote snapshot row → DB | UI ещё полируется отдельно; логика частично зависит от title-match |
| Public snapshot | `flooring-v2` flat rates, whitelist, local/remote prebuild | Spec lines отсутствуют; assembly rows не раскрываются в snapshot |
| Public calculation | `calculateFlooring` читает generated snapshot rates | Типы/опции фиксированы known codes; custom DB rows не становятся полноценным выбором |
| Spec | Общий estimate spec modal есть | Flooring section пока раскрывается плоскими line items, не реальными rows пакета |

**P0-риски, которые закрываются до расширения функциональности**

| Риск | Почему критично | Решение |
|---|---|---|
| Underlay promote из snapshot | `underlayPricePerM2` сейчас может быть сохранён как расход, а backend умножает расход на глобальные `220 ₽/м²`; это может раздуть подложку | При promote хранить `underlay_consumption_per_m2 = underlayPricePerM2 / DEFAULT_PUBLIC_UNDERLAY_PRICE_PER_M2`; добавить parity test |
| Неполный DB catalog не overlay-ит defaults | Если создан/изменён только `Ковролин`, snapshot может продолжать брать default, потому каталог не F1-complete | Snapshot builder должен overlay-ить known DB rows поштучно поверх defaults; incomplete catalog больше не должен блокировать известные правки |
| Orphan assemblies после delete target | Удаление covering/preparation/layout не связано FK с `catalog_assemblies` | Delete use-case/storage сначала удаляет assembly target, затем flat row; тест на отсутствие orphan rows |
| Fixed public codes | Admin может создать custom package, но public selector/types его не примут | Ввести динамические options из snapshot или временно явно запретить custom в public до FP4 |
| Assembly не участвует в spec | Пользователь не получает реальную смету с мешками/литрами/шт | Snapshot v3 добавляет public-safe `specLines`; public spec builder разворачивает rows по площади |
| Duplicate titles | `catalogId` и snapshot overlay местами матчятся по title | Добавить stable public code/source_code для catalog rows или запретить duplicate global titles |

**План FP0–FP8**

| Этап | Scope | Результат | Gate |
|---|---|---|---|
| **FP0** | Зафиксировать текущий as-built + P0-риски в документации | Единый план внедрения без UI-редизайна | docs-only commit |
| **FP1** | Safety fixes: underlay promote, partial overlay defaults, orphan assembly cleanup, duplicate-title guard/ошибка | Создание/редактирование известных строк безопасно отражается в snapshot; delete не оставляет мусор | pytest snapshot/routes/storage + vitest mappers/promote |
| **FP2** | Package projection: pure aggregation `assembly rows → flat projection` для covering/layout/preparation | Flat поля перестают быть ручной догадкой; пакет имеет проверяемый итог | unit tests formula/rounding; golden parity |
| **FP3** | Transactional save: create/edit package сохраняет flat projection + assembly rows согласованно | Нельзя сохранить пакет, где flat не соответствует rows, без явного override | vitest hook/helper + backend validation where needed |
| **FP4** | Snapshot v3: `flat rates + specLines`; v2 fallback сохраняется | Snapshot отдаёт расчётные ставки и публичную спецификацию без internal leakage | `test_public_flooring_snapshot_whitelist.py`, `public-flooring-snapshot.test.ts` |
| **FP5** | Public dynamic flooring options из snapshot | Custom packages из admin появляются в `/estimate`; known defaults остаются стабильными | public estimate tests + manual `/estimate` |
| **FP6** | Public flooring spec expansion | Спецификация полов раскрывает покрытие, расходники, работы, упаковки/округления | `estimate/spec.test.ts`, flooring golden spec |
| **FP7** | Backfill existing flat rows to assemblies + parity | Старые данные не теряются; суммы до/после совпадают | migration/backfill smoke + public golden totals |
| **FP8** | Release hardening: deployment checklist, Render prebuild remote snapshot, rollback notes | Готовность к production deploy | full pytest, admin-ui test/build, local smoke, remote prebuild fail/success |

**Требования к расчёту spec lines**

- Для строк с фасовкой считать both:
  - расчётное количество (`rawQuantity`);
  - закупочное количество (`purchaseQuantity`) с округлением вверх до упаковки.
- Для покрытия использовать площадь закупки с запасом.
- Для клея/СВП/затирки использовать площадь закупки, если строка зависит от резки/отхода; для грунта и инструмента — площадь работ, если не задано иначе.
- Для работ использовать площадь работ без запаса.
- Округление и единицы фиксируются в pure helpers, а не в UI.
- Public spec line не хранит внутренний `assembly_item_id`; допускаются только `code`, `title`, `category`, `quantity`, `unit`, `unitPrice`, `total`, `roomName`, `sourcePackageCode`.

**Data-loss safety**

- Все FP-этапы до FP7 additive: старые flat rows остаются в БД.
- Backfill только идемпотентный: повторный запуск не дублирует assemblies.
- Перед production FP7/FP8: snapshot текущей production DB / Render backup обязателен.
- Удаление target row должно быть явным пользовательским действием с подтверждением; автоматические миграции не удаляют пользовательские packages.
- При ошибке assembly save после flat save статус должен явно говорить о частичном успехе; rollback не выполняется без отдельного transactional design.

**Release gates для каждого этапа**

- Backend changes: targeted pytest + `pytest -q`.
- Frontend changes: `cd admin-ui && npm run test && npm run build`.
- Snapshot changes: local prebuild without env, remote-fail with invalid `PUBLIC_SNAPSHOT_BASE_URL`, remote-success against local backend.
- Public changes: `/estimate` manual smoke for known default room, custom flooring package, spec modal, print/download path.
- Admin changes: `/catalog-editor` manual smoke: create from snapshot → edit → save → reload → delete → no orphan assembly.

**Не делать в FP-треке**

- Не чинить визуальный дизайн полов внутри FP1–FP4.
- Не переводить public calculator на runtime backend.
- Не удалять legacy flat fields до FP7 parity.
- Не расширять preparation materials без отдельного продуктового решения.
- Не прятать ошибки snapshot/promote под silent fallback.
