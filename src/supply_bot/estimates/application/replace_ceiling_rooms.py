...
class ReplaceCeilingRoomCommand:
    room_id: int
    default_catalog_item_id: int | None
    is_enabled: object
...
class ReplaceCeilingRoomsUseCase:
    ...
    async def execute(self, command: ReplaceCeilingRoomsCommand) -> int:
        ...
        for index, room_payload in enumerate(command.rooms, start=1):
            ...
            catalog_item_id = room_payload.default_catalog_item_id
            ...
            rows.append(
                {
                    ...
                    "ceiling_area_m2": optional_non_negative(room_payload.ceiling_area_m2),
                    "area_source": _normalize_optional_payload_text(room_payload.area_source) or "room_area",
                    "perimeter_m": optional_non_negative(room_payload.perimeter_m),
                    ...
                    "sort_order": int(room_payload.sort_order or index * 10),
                }
            )
